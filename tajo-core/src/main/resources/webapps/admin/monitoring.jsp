  ...
  $.tajoMonitor.parser = {
    parseSharedPie: function(meta, v, k){
      ...
      if(meta.success){
        ...
        $.each(v, function(index, value) {
          try{
            var __v;
            var q = meta.metrics[v[index]].queue;
            $.each(q[q.length-1], function(index, value) {
              __v = value;
              return;
            });
            _v.push(__v);
    ...
    parseMasterClusterUptime: function(meta){
      if(meta.success){
        try{
          var __v = 1000;
          var q = meta.metrics['MASTER.CLUSTER.UPTIME'].queue;
          $.each(q[q.length-1], function(index, value) {
            __v = value;
            return;
          });
          return parseInt(__v/1000);
        }catch(e){
          console.log(e);
        }
      }
      ...
      if(meta.success){
        ...
        for (i = 0; i < k.length; i++) {
          try{
            var _v = [];
            var __v;
            var q = meta.metrics[k[i]].queue;
            $.each(q, function(index, value) {
              $.each(value, function(ii, vv) {
                var _val = 0;
                __v = vv;
                if(type == $.tajoMonitor.TYPE.PERCENT){
                  _val = parseInt(__v * 100);
                } else {
                  _val = __v;
                }
                ii = Number(ii);
                var mv = makeValue(ii, _val); //{x:timestamp, y:value}
                _v.push(mv); //[{x:timestamp, y:value}...]
                return;
              });
            });
            var item = {key: k[i], values: _v};

            //console.log(k[i], " : ", item.values.length);
    ...
    parseSharedUsage: function(meta, k){
      ...
      if(meta.success){
        ...
        try{
          ...
          var q = meta.metrics[k[0]].queue;
          $.each(q[q.length-1], function(index, value) {
            __v = value;
            return;
          });
          var v = parseInt(__v*100);
          _k.push(k[0]);
          if(k.length>1){
            _k.push(k[1]);
          }
